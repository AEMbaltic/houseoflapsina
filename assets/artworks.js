...
(function (global) {
  ...
  works.forEach(function (art, i) {
    ...
    // or if it is not there at all — the drawn version stands in for it.
    if (art.src) {
      const img = new Image();
      img.onload = function () {
        art.image = img;
        window.dispatchEvent(new CustomEvent('lapsina:art', { detail: { id: art.id } }));
      };
      img.onerror = function () { /* no photograph yet: the drawing stands in */ };
      img.src = art.src;
    }
